...
exrLoader.load("/static/core/resources/textures/skybox.exr", (texture) => {
...
gltfLoader.load("/static/core/resources/models/hall.glb", (gltf) => {
  const hall = gltf.scene;
  scene.add(hall);

  const pokerTablePos = hall.getObjectByName("PokerTable_Position");
  if (pokerTablePos) {
    gltfLoader.load("/static/core/resources/models/table.glb", (tableGltf) => {
      ...
      fontLoader.load(
        ...
        (font) => {
          ...
          interactText.position.set(
            pokerTable.position.x,
            pokerTable.position.y + 3,
            pokerTable.position.z
          );
